...
#email: constantinoantonio.garcia [at] usc.es
date: "`r Sys.Date()`"
...
not the completely phase space. For example, let's assume that we have only measured
...
be computed by using the well-known Cao's algorithm [@cao1997practical]:
...
step manifests a linear behaviour. The slope of this linear region yields the
...
In the following sections we illustrate this procedure computating the correlation
dimension, the sample entropy and the Lyapunov exponents of the Lorenz system.
For a theoretical discussion of these methods, the interested reader is referred
to [@kantz2004nonlinear].
...
specify the range in which the linear behaviour appears (`regression.range`
...
exponent computes the divergence rates of close trajectories in the phase space
of a time series. To define what is a close trajectory we make use of the `radius` 
