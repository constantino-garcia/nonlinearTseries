...
NumericMatrix rcpp_information_dimension(const NumericVector& timeSeries,
                                         ...
                                         int nBoxes, int nReferenceVectors,
                                         int theilerWindow, int nNeighbours){
  ProfilerStart("/tmp/infDimProf.txt");
...
# Henon
ts = henon(n.sample =  1000, n.transient = 100, a = 1.4,
           b = 0.3, start =  c(0.8253681, 0.6955566),
           do.plot = FALSE)$x

radius = 1e-3

eps = infDim(ts, min.embedding.dim = 2, max.embedding.dim =2,
                  time.lag = 1,
                  min.fixed.mass = 0.04, max.fixed.mass = 0.2,
                  number.fixed.mass.points = 5,
                  radius = radius, increasing.radius.factor = sqrt(2),
                  number.boxes = 100, number.reference.vectors = 100,
                  theiler.window = 10, kMax = 100, do.plot = FALSE)
message("*************")
leps = rcppInfDim(ts, min.embedding.dim = 2, max.embedding.dim =2,
                  time.lag = 1,
                  min.fixed.mass = 0.04, max.fixed.mass = 0.2,
                  number.fixed.mass.points = 5,
                  radius = radius, increasing.radius.factor = sqrt(2),
                  ...
                  theiler.window = 10, kMax= 100, do.plot = FALSE)
library(testthat)
testthat::expect_equal(eps,leps)
library(microbenchmark)
print(
  microbenchmark(R = infDim(ts, min.embedding.dim = 2, max.embedding.dim =2,
                            time.lag = 1,
                            min.fixed.mass = 0.04, max.fixed.mass = 0.2,
                            number.fixed.mass.points = 5,
                            radius = radius, increasing.radius.factor = sqrt(2),
                            number.boxes = 100, number.reference.vectors = 100,
                            theiler.window = 10, kMax= 100, do.plot = FALSE),
                 C = rcppInfDim(ts, min.embedding.dim = 2, max.embedding.dim =2,
                                time.lag = 1,
                                min.fixed.mass = 0.04, max.fixed.mass = 0.2,
                                number.fixed.mass.points = 5,
                                radius = radius, increasing.radius.factor = sqrt(2),
